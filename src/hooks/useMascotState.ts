...
export type MascotExpression = 'normal' | 'blink' | 'happy' | 'sleepy' | 'excited' | 'blush' | 'angry'
...
export function useMascotState() {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      ;[inactivityTimer, blinkTimer, symbolTimer, stateTimer, scrollStopTimer, expressionTimer].forEach(
  ...
  }, [transitionTo, safeSet, resetInactivity])

  const triggerBlush = useCallback(() => {
    if (currentState.current === 'angry') return
    resetInactivity()
    if (expressionTimer.current) clearTimeout(expressionTimer.current)
    transitionTo('blush', 3000)
    safeSet((prev) => ({ ...prev, expression: 'blush' }))
    expressionTimer.current = setTimeout(() => {
      if (mounted.current) safeSet((prev) => ({ ...prev, expression: 'normal' }))
    }, 3000)
  }, [transitionTo, safeSet, resetInactivity])
  ...
  const triggerAngry = useCallback(() => {
    resetInactivity()
    if (expressionTimer.current) clearTimeout(expressionTimer.current)
    transitionTo('angry', 3500)
    safeSet((prev) => ({ ...prev, expression: 'angry' }))
    expressionTimer.current = setTimeout(() => {
      if (mounted.current) safeSet((prev) => ({ ...prev, expression: 'normal' }))
    }, 3500)
  }, [transitionTo, safeSet, resetInactivity])
  ...
  return { ...stateData, triggerHappy, triggerTyping, stopTyping, triggerBlush, triggerAngry }
}
